...
If you discover a security issue in cloud-itonami-isic-0240, **do not open a
...
This repository implements the **Governor** for forestry-support-service
...
1. **Hard violations in Governor rules must never be overridable** — a rule
   marked as HARD (e.g., `:already-logged`, `:operator-certification-expired`,
   `:forestry-equipment-or-fire-tactic-decision-blocked`) blocks the proposal
   unconditionally
2. **Escalation gates must trigger without race conditions** — forest-health
...
- Include documentation of the forestry-support-service safety/regulatory rationale
